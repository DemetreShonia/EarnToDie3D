...
//    ,float densityScale, float noiseScale, float depth, float3 lightDirection, 
//    float lightAbsorb, float darknessThreshold, float transmittance,out float2 Out)
//{
...
//        ray += rayDirection * stepSize;
//
//        float sampledDensity = snoise(ray * noiseScale) + 0.5f * snoise(ray * noiseScale * 4);
...
//            lightRay += -1 * lightDirection * stepSize;
//            float lightDensity = snoise(lightRay * noiseScale) + 0.5f * snoise(lightRay * noiseScale * 4);
//            lightAcc += lightDensity * densityScale;
//        }
//
//        float lightTransmission = exp(-lightAcc);
...
//    }
//    Out = float2(finalLight,exp(-density));
//    
//}